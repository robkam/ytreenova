...
int LogArchiveFile(ViewContext *ctx, YtreeNovaPanel *panel,
                   const FileEntry *file_entry) {
  char archive_member_path[PATH_LENGTH + 1];
  char canonical_member_path[PATH_LENGTH + 1];
  char archive_root_path[PATH_LENGTH + 1];
  char entry_dir_path[PATH_LENGTH + 1];
  char temp_path[PATH_LENGTH + 1];
  char logical_archive_path[PATH_LENGTH + 1];
  const char *relative_dir;
  size_t relative_len;
  int fd = -1;
  int written;

  if (!ctx || !panel || !panel->vol || !file_entry ||
      ctx->view_mode != ARCHIVE_MODE)
    return -1;

  GetPath(file_entry->dir_entry, entry_dir_path);
  GetPath(panel->vol->vol_stats.tree, archive_root_path);
  relative_dir = entry_dir_path;
  if (strncmp(entry_dir_path, archive_root_path, strlen(archive_root_path)) ==
      0) {
    relative_dir += strlen(archive_root_path);
    if (*relative_dir == FILE_SEPARATOR_CHAR)
      relative_dir++;
  }

  written = snprintf(archive_member_path, sizeof(archive_member_path), "%s%s%s",
                     relative_dir, *relative_dir ? FILE_SEPARATOR_STRING : "",
                     file_entry->name);
  if (written < 0 || (size_t)written >= sizeof(archive_member_path))
    return -1;
  relative_len = strlen(archive_member_path);
  if (relative_len == 0 ||
      Archive_ValidateInternalPath(archive_member_path, canonical_member_path,
                                   sizeof(canonical_member_path)) != 0)
    return -1;

  written = snprintf(logical_archive_path, sizeof(logical_archive_path), "%s/%s",
                     panel->vol->vol_stats.display_path, canonical_member_path);
  if (written < 0 || written >= PATH_LENGTH)
    return -1;

  if (!Path_CreateTempFile(temp_path, sizeof(temp_path), "ytnova_archive_log_",
                           FALSE, &fd))
    return -1;
  if (ExtractArchiveEntry(panel->vol->vol_stats.log_path, canonical_member_path,
                          fd, ctx->hook_archive_callback, ctx) != 0) {
    close(fd);
  ...
  if (LogDisk(ctx, panel, temp_path) != 0) {
    (void)unlink(temp_path);
    return -1;
  }
  (void)snprintf(panel->vol->vol_stats.temporary_archive_path,
                 sizeof(panel->vol->vol_stats.temporary_archive_path), "%s",
                 temp_path);
  (void)snprintf(panel->vol->vol_stats.display_path,
                 sizeof(panel->vol->vol_stats.display_path), "%s",
                 logical_archive_path);
  memcpy(panel->vol->vol_stats.tree->name, logical_archive_path,
         (size_t)written + 1);
  if (ctx->hook_display_disk_statistic)
    ctx->hook_display_disk_statistic(ctx, &panel->vol->vol_stats);
  return 0;
