...
*   **Read-only Archive Contract:** A read-only archive replaces the fixed-width `COMMANDS` signpost with `READONLY` on its directory, file, and preview footers. Writable archives retain `COMMANDS`, so command positions do not shift.
...
*   **Audit Inventory:**
    *   **Filesystem `F7`/preview:** Missing: None. Inapplicable: None. The direct runtime footer excludes `Hex`, `Log`, `Volume`, and `Sort` through the established preview filter; `Global` and `Showall` are file-list states, not preview actions. Repair: preview packing must consume the capability-filtered count returned by `ResolveFooterCommandList()`.
    *   **Archive directory:** Missing: `Invert` from archive directory, although the shared directory dispatch supports it and it has no archive capability requirement. Inapplicable: None. `Attributes`, `Newfile`, `Execute`, and `Archive` are correctly absent because no archive-directory dispatch supports them. Writable runtime footer retains `Copy`, `Delete`, `Makedir`, `MoveDir`, `Pathcopy`, and `Rename`; direct read-only runtime footer retains `Copy` and `Pathcopy` while omitting all mutations. Repair: add `Invert`; use the resolved command count when packing the footer.
    *   **Archive file, including tagged, `Global`, and `Showall` states:** Missing: None. Inapplicable: None. Direct runtime output omits filesystem-only `Attributes`, `Edit`, `Newfile`, `Execute`, and `Archive`; it retains the supported tagged and untagged copy, delete, move, rename, output, view, and pathcopy actions. `Global` and `Showall` remain file-list states and do not create a separate footer surface. Repair: use the resolved command count when packing the footer.
    *   **Archive `F7`/preview:** Missing: None. Inapplicable: `Attributes`, `Edit`, `Newfile`, `Execute`, and `Archive` are offered by direct runtime output although archive-file footer omits them and archive dispatch does not implement them. Repair: make archive preview exclude the archive-file-inapplicable actions and use the resolved command count when packing.
    *   **Universally unsupported VFS operations:** `Attributes`, `Edit`, `Newfile`, `Execute`, and `Archive` are absent from archive directory/file footers; archive preview is the sole surface that still advertises them.
*   **Repair Family for Task 92.2:** Use `ResolveFooterCommandList()`'s returned count at each packing call, add archive-directory `Invert`, and share archive-file inapplicability filtering with archive preview. Prove the writable/read-only and tagged/untagged matrix against dispatch.
*   - [x] **Status:** Completed.
...
*   **Final Inventory:**
    *   **Filesystem directory and file:** Retain the maintainer-checked baseline command sets and their existing directory/file dispatch paths.
    *   **Filesystem `F7`/preview:** Retains the applicable file actions and excludes `Hex`, `Log`, `Volume`, and `Sort`; packing consumes the resolved command count.
    *   **Writable archive directory:** Adds `Invert`; retains supported copy, mutation, navigation, tagging, output, and filtering actions; uses the resolved command count and `COMMANDS` signpost.
    *   **Writable archive file:** Retains supported tagged and untagged copy, delete, move, rename, output, view, and pathcopy actions. `Global` and `Showall` use this same file command inventory.
    *   **Writable archive `F7`/preview:** Matches the archive file applicability contract and excludes `Attributes`, `Edit`, `Newfile`, `Execute`, and `Archive` while retaining supported preview actions.
    *   **Read-only archive directory, file, and `F7`/preview:** Retain browse, copy-out, tag, filter, output, pipe, view, pathcopy, and navigation actions where applicable; omit capability-gated mutations and use the fixed-width `READONLY` signpost.
*   - [x] **Status:** Completed.
...
*   **Execution Source of Truth:** This task is the sole plan and recovery record for the work.
...
*   **Single-Section Schema Contract:** Every F1 topic contains metadata followed by exactly one `### Contextual F1` section. All necessary topic links occur naturally inside that section. The generated runtime model stores only topic identity, title, contexts, and contextual text. Runtime navigation follows links extracted from that text. The generator validates inline-link syntax, existing targets, self-links, locale target parity, and index organisation.
...
*   **Information-Architecture Contract:** `Help Index` lists `Navigation`, which opens the `YtreeNova Navigation` topic, not `F1 Navigation`. `F1 Navigation` is reached from the help strip’s integrated `Navigation` command, whose highlighted `N` is the binding; its label is not `N Navigation`. `Help Index` does not show its own `I Index` command. Preserve this distinction. The information-architecture audit includes `execute-dir` and `execute-file` in the index because they own non-obvious `{}` placeholder, shell-syntax, and tagged-scope behavior, consistent with the other indexed prompt topics. This is a decision about those two pages, not a blanket every-topic index policy; `F1 Navigation` remains outside the index because the help strip owns its direct route. Each local page must answer: what this screen or prompt is for; what the user can do here; what local exception or trap matters; and where the one deeper explanation lives if needed. A reusable explanation has one owner. Local pages give only the local consequence and one direct link. Do not create prompt pages unless the prompt owns non-obvious syntax, scope, placeholders, wildcard behavior, or special controls that users need at input time.
*   **Corpus Rewrite Contract:** Revalidate the six established examples (`f1-navigation`, `ytnova-navigation`, `tagged`, `copy-move-targets`, `directory`, and `file`) against current behaviour and correct factual, grammatical, or markup defects. Rewrite the other 36 non-index topics from blank, grouped by behavioural family rather than by polishing their existing prose. Rewrite `Help Index` last, after topic ownership, titles, and links are settled.
*   **Implementation Batches:** The single-section schema/runtime migration and the editorial corpus rewrite may be delivered as separate coherent, green implementation batches because they have different risks and validation paths, but both remain part of this task.
...
*   **Presentation Contract:** `F1` text must remain readable when the popup wraps at narrow terminal widths. This does not impose a fixed topic width; wrapping continues to follow the available terminal size. Backticked terms and distinct term/definition labels that users need to identify must visibly use `help_topic`. Mnemonic-highlighted help-strip commands must visibly use configured `help_keybind` styling. Plain-text help-strip key-token labels such as `Enter/Right` and `Esc/Quit` must use `help_footer`. Help presentation must stay contextual and readable without depending on right-edge clipping or hard-coded color assumptions.
*   **Acceptance Criteria:**
    ...
    *   Every topic has exactly one `### Contextual F1` section, with all necessary links inline.
    *   Inline-link syntax, targets, self-links, locale parity, and index organisation are generator-validated.
    *   Generated topics contain only the fields consumed by inline contextual help.
    ...
    *   Every topic remains usable at normal and narrow terminal widths, with wrapping determined by the available terminal size.
    *   Required generation, clean build, focused source/generator/locale tests, and runtime presentation checks pass.
- [x] **Status:** Completed.
