...
*   **Control-Alias Canonicalization Rule:** Terminal-equivalent aliases (`^m`/Enter/CR, `^j`/LF enter path, and `^[`/Esc) are a single canonical input for binding and validation. A confirmed kitty keyboard protocol path distinguishes only `^m` from Enter for tagged move.
*   **Keyboard Portability Baseline:** ytnova negotiates kitty keyboard protocol disambiguation through the active local, multiplexer, or remote terminal path. A confirmed path uses `^m` for Move tagged. An unavailable, rejected, or malformed negotiation silently keeps curses `getch`/`KEY_*` semantics and `^n` for Move tagged (practical references: [`curs_getch(3x)`](https://man7.org/linux/man-pages/man3/curs_getch.3x.html) and [`terminfo(5)`](https://man7.org/linux/man-pages/man5/terminfo.5.html)).
...
*   **The Log Target Rule (`L`):** The filesystem file-window `Log Path:` prompt accepts a directory or a path recognized by a registered virtual-filesystem loader; it rejects ordinary unsupported files without changing the logged volume. Archive-file `Log` loads the selected member only when that member is recognized by a registered loader. Archive recognition is content-based, not extension-based. Nested archive statistics display the logical source chain (`outer-archive/member-archive`), never the temporary extraction path.
...
*   **F1 Section Rule:** Every F1 topic block contains exactly one `### Contextual F1` section. Followable topic links occur inline within that section, and the `Contents` return target MUST remain reachable from every help page through the help strip.
*   **Reference Section Rule:** Every man/USAGE topic block contains ordered `####` reference subsections directly after its metadata, written in terse reference tone. The reference source is markdown-editable for translators and maintainers and does not inherit F1 teaching prose or schema by generation trickery.
...
*   **Topic Link Rule:** Shared topics are linked only with Markdown links of the form `[Label](topic:<id>)` inside `### Contextual F1`. F1 reuse must link to shared topics rather than duplicate the same prose into multiple contextual pages, and link depth must remain shallow (one or two hops maximum from the starting page).
*   **Shallow-Path Rule:** Help links must keep the user oriented. Contextual pages should link sparingly to the topic that owns a necessary deeper explanation. The help strip provides the route to `Help Index`; authored topics do not need to repeat it. Command topics must absorb their common caveats instead of forcing a branching maze of follow-on links unless a subtopic is reused across multiple command families.
*   **F1 Index Rule:** `Help Index` links exactly once to every operator topic and orders its localized labels alphabetically. `Help Navigation` and `Start Here` are help-strip-only exceptions because the integrated `Navigation` and `Intro` commands open them directly. The index label `Navigation` opens `YtreeNova Navigation`. `Execute Directory` and `Execute File` remain indexed because they own non-obvious prompt syntax and scope.
*   **Required Topic Set Rule:** The topic inventory must include `index`, `f1-navigation`, `start-here`, `ytnova-navigation`, `list-jump`, `shared-commands`, `tagged`, `tagged-viewer`, `command-line-editing`, `command-line-parameters`, `configuration-files`, `copy-move-targets`, `vi-keys`, `f10`, `theming`, `directory`, `file`, `archive-dir`, `archive-file`, `filter`, `compare`, `compare-target`, `change-date`, `compare-scope`, `compare-basis`, `compare-results`, `execute-file`, `execute-dir`, `search-tagged`, `create-archive`, `output`, `output-format`, `output-destination`, `output-separator`, `showall`, `global`, `f7`, `f8`, `f8-dir`, `f8-file`, `history-dialog`, `volume-menu`, `applications-menu`, and `f2-picker`.
