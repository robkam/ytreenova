...
*   **F1 Index Rule:** `Help Index` links exactly once to every operator topic and orders its localized labels alphabetically. `Help Navigation` is the sole exception because the help strip's integrated `Navigation` command opens it directly. The index label `Navigation` opens `YtreeNova Navigation`. `Execute Directory` and `Execute File` remain indexed because they own non-obvious prompt syntax and scope.
*   **Required Topic Set Rule:** The topic inventory must include `index`, `f1-navigation`, `ytnova-navigation`, `list-jump`, `shared-commands`, `tagged`, `tagged-viewer`, `command-line-editing`, `command-line-parameters`, `configuration-files`, `copy-move-targets`, `vi-keys`, `f10`, `theming`, `directory`, `file`, `archive-dir`, `archive-file`, `filter`, `compare`, `compare-target`, `change-date`, `compare-scope`, `compare-basis`, `compare-results`, `execute-file`, `execute-dir`, `search-tagged`, `create-archive`, `output`, `output-format`, `output-destination`, `output-separator`, `showall`, `global`, `f7`, `f8`, `f8-dir`, `f8-file`, `history-dialog`, `volume-menu`, `applications-menu`, and `f2-picker`.
