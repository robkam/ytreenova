...
*   **Help Topic Schema Rule:** F1 topic blocks use an exact `## topic:<id>` heading, an immediate `ytnova-help-meta` fence with `title:` and `contexts:`, and exactly one `### Contextual F1` section. Followable topic links use `[Label](topic:<id>)` inline within that section. Man topic blocks independently use the heading and metadata fence followed directly by ordered level-4 reference subsections. Help topics without a direct runtime context declare `contexts: none`.
...
*   **Runtime Help Consumer Boundary:** `src/ui/runtime_help.c` is the sole UI consumer of `src/core/generated_help_topics.h`. It resolves context IDs to generated topics, appends runtime-owned footer/prompt command rows for parity, and lets users follow inline links across shallow topic paths without re-embedding help prose in controllers.
